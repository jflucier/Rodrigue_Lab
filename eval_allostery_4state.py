...
def motif_results(motif_out_dir, motif_pdb, motif):
    ...
    # Group statistics across the bounded states
    ...
    # Flattened multi-state dataframe pivot
    ...
    # Text label replacements for your first 4 regulatory environments
    ...
    print(f"Test Successful! Processed states 0-3 and saved to: {motif_out_dir}/_aggresults.csv")
...
def do_single_dir(design_dir):
    ...
    # 🔥 TEST RESTRICTION: Rigidly evaluate only states 0, 1, 2, and 3 to ensure standard lengths
    target_states = [0, 1, 2, 3]
    ...
    for state in target_states:
        ...
        for sample_idx in range(5):  # 5 samples per design model tracking
            for seqid in range(1, 9):  # Sequence mapping loops
                if args.lmpnn:
...
if __name__ == "__main__":
    ...
    for motif in os.listdir(out_dir):
        ...
        if not os.path.isdir(motif_out_dir):
            continue
        print(f"Executing bounded test calculation loop for motif target: {motif}")

...
# Fixed template using clean Hydra list syntax without nested single quotes
...
def parse_hotspots(hotspot_str: str):
    ...
    chains = set()
    for tok in tokens:
        chains.add(tok[0]) # Extract chain identifier character
    if len(chains) != 1:
        raise ValueError(
...
def main():
    ...
    with open(args.targets_tsv, newline="") as fh:
        ...
        for row in reader:
            ...
            # Clean list format that Hydra accepts: [A25,A26,A33] instead of [\'A25\',\'A26\']

...
Automatically deduces if previous outputs exist in the design folder and adjusts
num_designs and design_startnum dynamically using correct Hydra syntax.
...
# Corrected to use official Hydra flag name: inference.design_startnum
...
def find_next_start_num(design_out_dir: Path) -> int:
    """Scans the design folder for existing outputs and returns the highest index found + 1."""
...
def main():
    ...
    with open(args.targets_tsv, newline="") as fh:
        ...
        for row in reader:
            ...
            # Auto-deduce correct start index offset and total remaining runs
            design_startnum = find_next_start_num(design_out_dir)
            needed_designs = args.num_designs - design_startnum

            if needed_designs <= 0:
                print(f"[{name}] Already has {design_startnum} designs produced. Target threshold reached. Skipping.")
                continue

            if design_startnum > 0:
                print(
                    f"[{name}] Detected {design_startnum} finished designs. Resuming generation for remaining {needed_designs} entries starting at index {design_startnum}.")
    ...
    print(f"\nWrote {len(written)} containerized run script(s) to {out_dir}/")
    for p in written:
        print(f"  {p}")

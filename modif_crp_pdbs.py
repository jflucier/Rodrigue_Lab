...
def main():
    parser = argparse.ArgumentParser(description="Inject valid Switchcraft length-padded REMARK 999 headers and filter atoms.")
    ...
    # Filter structural lines to include ONLY the specified motif residues
    motif_atom_lines = []
    motif_ranges = [seg for seg in architecture if seg["type"] == "motif"]

    for line in clean_lines:
        if line.startswith("ATOM  ") or line.startswith("HETATM"):
            # Extract chain ID (column 22 / index 21 in PDB format)
            chain_id = line[21].strip()
            try:
                # Extract residue sequence number (columns 23-26 / indices 22-26)
                res_seq = int(line[22:26].strip())
            except ValueError:
                continue

            # Check if this atom falls inside any target motif windows for Chains A or B
            ...
            # Preserve structural metadata like TER or END lines if encountered
            if line.startswith("TER") or line.startswith("END"):
                motif_atom_lines.append(line)

    # Write out the structural file containing the headers and pruned atoms
    with open(args.output, 'w') as f:
        f.write(final_header)
        f.writelines(motif_atom_lines)

    print(f"Success! Scaled template (138 residues filtered from native PDB) saved to: {args.output}")
